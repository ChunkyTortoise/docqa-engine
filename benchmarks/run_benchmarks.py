...
def evaluate_results(results: list[SearchResult], keywords: list[str], k: int = 5) -> dict:
...
def format_table(results: list[dict], k: int) -> str:
    ...
    for r in results:
        lines.append(f"| {r['method']} | {r['precision_at_k']:.1%} | {r['hit_rate']:.1%} | {r['avg_time_ms']:.1f}ms |")
    lines.extend(
        [
            "",
